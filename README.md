...
var rules = Object.assign({}, SimpleMarkdown.defaultRules, {
    footnote: {
        ...
        match: function(source) {
            ...
            // This regex matches footnotes at the current source position (^):
            // match a `^[` followed by a captured group of either
            //    `\.` (backslash anything) or
            //    a non-`\`, non-`]` character
            // then a `]`
            return /^\^\[(?:\\.|[^\\\])]+)\]/.exec(source);
        },

        parse: function(capture, recursiveParse, state) {
            // save the current footnote:
            // reserve a spot for the current footnote:
            state.footnotes = state.footnotes || [];
            var footnoteIndex = state.footnotes.length++;

            // recursively parse the footnote text, and save it:
            var footnote = state.footnotes[footnoteIndex] = {
                type: 'footnote',
                index: footnoteIndex,
                content: recursiveParse(capture[1], state),
            };

            // We can then use the object both as the footnote node,
            // and maintain a reference to it on the state, for outputting
            // separately. This lets us access it in both places we may need it
            return footnote;
        },
...
rule via customization. For example, to make `### headings` support a new block
on the next line, as in [issue #30][issue30], you can customize the heading
rule:

[issue30]: https://github.com/Khan/simple-markdown/issues/30
...
var rules = Object.assign({}, SimpleMarkdown.defaultRules, {
    heading: Object.assign({}, SimpleMarkdown.defaultRules.heading, {
        match: SimpleMarkdown.blockRegex(
            ...
            // followed by optional empty lines (required in defaultRules),
...
MIT. See the [LICENSE file][LICENSE] for text.
